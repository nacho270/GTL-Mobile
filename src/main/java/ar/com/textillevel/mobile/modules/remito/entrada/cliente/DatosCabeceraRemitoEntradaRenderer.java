...
public class DatosCabeceraRemitoEntradaRenderer extends DatosCabeceraRenderer<RemitoEntradaMobileTO> {
	...
	public void render() {
		setTextOnTextView(R.id.re_nro, "REMITO DE ENTRADA N " + getDocument().getNroRemito(), getRootView());
		...
		setTextOnTextView(R.id.re_owner,"SR/ES: " + getDocument().getOwner().getRazonSocial(),getRootView());
		setTextOnTextView(R.id.re_owner_dir,"DIRECCION: " + getDocument().getOwner().getDireccion() + " - " + getDocument().getOwner().getLocalidad(),getRootView());
