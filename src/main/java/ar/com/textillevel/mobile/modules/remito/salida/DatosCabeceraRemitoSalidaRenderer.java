...
public class DatosCabeceraRemitoSalidaRenderer extends DatosCabeceraRenderer<RemitoSalidaMobileTO> {
	...
	public void render() {
		setTextOnTextView(R.id.rs_nro, "REMITO N " + getDocument().getNroRemito(), getRootView());
		...
		setTextOnTextView(R.id.rs_owner,"SR/ES: " + getDocument().getOwner().getRazonSocial(),getRootView());
		setTextOnTextView(R.id.rs_owner_dir,"DIRECCION: " + getDocument().getOwner().getDireccion() + " - " + getDocument().getOwner().getLocalidad(),getRootView());
