...
public class DatosCabeceraRemitoEntradaProveedorRenderer extends DatosCabeceraRenderer<RemitoEntradaProvMobileTO> {
	...
	public void render() {
		setTextOnTextView(R.id.re_nro_p, "REMITO DE ENTRADA N " + getDocument().getNroRemito(), getRootView());
		setTextOnTextView(R.id.re_fecha_p,"FECHA: " + getDocument().getFecha(),getRootView());
		setTextOnTextView(R.id.re_owner_p,"SR/ES: " + getDocument().getOwner().getRazonSocial(),getRootView());
		setTextOnTextView(R.id.re_owner_dir_p,"DIRECCION: " + getDocument().getOwner().getDireccion() + " - " + getDocument().getOwner().getLocalidad(),getRootView());
