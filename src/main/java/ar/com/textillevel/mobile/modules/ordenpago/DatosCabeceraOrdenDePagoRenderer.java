...
public class DatosCabeceraOrdenDePagoRenderer extends DatosCabeceraRenderer<OrdenDePagoMobileTO> {
	...
	public void render(){
		...
		setTextOnTextView(R.id.owner_odp, "SE?ORES: " + getDocument().getCliente().getRazonSocial(),getRootView());
		setTextOnTextView(R.id.owner_odp_dir,"DIRECCI?N: " + getDocument().getCliente().getDireccion() + " - " + getDocument().getCliente().getLocalidad(),getRootView());
