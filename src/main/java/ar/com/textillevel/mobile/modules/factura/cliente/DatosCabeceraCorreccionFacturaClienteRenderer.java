...
public class DatosCabeceraCorreccionFacturaClienteRenderer extends DatosCabeceraRenderer<CorreccionFacturaMobTO> {
	...
	public void render() {
		...
		setTextOnTextView(R.id.factura_tipo, (esNC ? "NOTA DE CREDITO" : "NOTA DE DEBITO") + " - N " + getDocument().getNroCorreccion(), getRootView());
		setTextOnTextView(R.id.factura_fecha,"FECHA: " + getDocument().getFecha(),getRootView());
		setTextOnTextView(R.id.factura_owner,"SR/ES: " + getDocument().getCliente().getRazonSocial(),getRootView());
		setTextOnTextView(R.id.factura_owner_dir,"DIRECCION: " + getDocument().getCliente().getDireccion() +  (getDocument().getCliente().getLocalidad() == null ? " " : " - " + getDocument().getCliente().getLocalidad()),getRootView());
		if(esNC) {
